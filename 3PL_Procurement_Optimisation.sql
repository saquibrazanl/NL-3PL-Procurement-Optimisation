...
-- Tools:   SQL (PostgreSQL compatible) | Excel | Power BI
-- ============================================================
...
-- Drop tables if rebuilding (safe order due to FK dependencies)
...
-- ----------------------------
-- TABLE 1: providers
-- The 3 logistics providers being evaluated
-- ----------------------------
CREATE TABLE providers (
    provider_id     SERIAL PRIMARY KEY,
    provider_name   VARCHAR(100) NOT NULL,
    country_hq      VARCHAR(50),
    provider_type   VARCHAR(50),   -- 'Road', 'Air', 'Multimodal'
    founded_year    INT,
    eu_presence     BOOLEAN DEFAULT TRUE,
    sla_threshold   DECIMAL(5,2),  -- Minimum on-time % required in contract
    notes           TEXT
);

-- ----------------------------
-- TABLE 2: routes
-- Key NL → EU trade corridors
-- ----------------------------
CREATE TABLE routes (
    ...
    distance_km     INT,
    route_type      VARCHAR(50),  -- 'Road', 'Rail', 'Multimodal'
    avg_lead_days   DECIMAL(4,1)  -- Market average lead time in days
);

-- ----------------------------
-- TABLE 3: quotes
-- Provider rate quotes per route per shipment weight band
-- ----------------------------
CREATE TABLE quotes (
    quote_id            SERIAL PRIMARY KEY,
    provider_id         INT REFERENCES providers(provider_id),
    route_id            INT REFERENCES routes(route_id),
    weight_band_kg      VARCHAR(20),   -- e.g. '0-100', '100-500', '500+'
    base_rate_eur_per_kg DECIMAL(8,4),
    fuel_surcharge_pct  DECIMAL(5,2),  -- % added on top of base rate
    volume_discount_pct DECIMAL(5,2),  -- % discount at high volume
    min_shipment_kg     DECIMAL(8,2),
    effective_date      DATE,
    expiry_date         DATE,
    quote_source        VARCHAR(100)   -- e.g. 'Freightos', 'Direct negotiation'
);

-- ----------------------------
-- TABLE 4: shipments
-- Historical shipment records (actual transactions)
-- ----------------------------
CREATE TABLE shipments (
    ...
    status              VARCHAR(30),   -- 'Delivered', 'Delayed', 'Lost', 'Damaged'
...
-- Monthly aggregated KPI records per provider per route
-- ----------------------------
CREATE TABLE performance_history (
    perf_id             SERIAL PRIMARY KEY,
    provider_id         INT REFERENCES providers(provider_id),
    route_id            INT REFERENCES routes(route_id),
    month_year          DATE,          -- First day of month (e.g. 2024-01-01)
    total_shipments     INT,
    on_time_count       INT,
    delayed_count       INT,
    damaged_count       INT,
...
-- Quotes (base rates based on Freightos/public benchmarks, 2024)
-- DHL Freight quotes (provider_id = 1)
...
-- DB Schenker quotes (provider_id = 2)
INSERT INTO quotes (provider_id, route_id, weight_band_kg, base_rate_eur_per_kg, fuel_surcharge_pct, volume_discount_pct, min_shipment_kg, effective_date, expiry_date, quote_source) VALUES
...
-- Kuehne+Nagel quotes (provider_id = 3)
INSERT INTO quotes (provider_id, route_id, weight_band_kg, base_rate_eur_per_kg, fuel_surcharge_pct, volume_discount_pct, min_shipment_kg, effective_date, expiry_date, quote_source) VALUES
...
-- Shipments (60 records across 2024 — realistic mix)
...
-- DHL, AMS→Frankfurt (Route 1), Jan-Apr 2024
...
-- DB Schenker, AMS→Frankfurt (Route 1)
...
-- DB Schenker, Rotterdam→Paris (Route 2)
...
-- DB Schenker, Eindhoven→Warsaw (Route 3) -- DB Schenker is strongest on this lane
...
-- Kuehne+Nagel, AMS→Frankfurt (Route 1)
...
-- Kuehne+Nagel, Rotterdam→Paris (Route 2) -- KN strongest here
...
-- Kuehne+Nagel, Eindhoven→Warsaw (Route 3)
...
-- Performance history (monthly summaries Jan–Jun 2024)
...
(1,3,'2024-02-01',9,7,2,0,3.9,0.4250,2340.00,FALSE),  -- Feb slight SLA risk
...
-- Use case: Compare actual landed cost per provider per route
-- ----------------------------
SELECT
    p.provider_name,
    r.origin_city || ' → ' || r.dest_city AS lane,
    q.weight_band_kg,
...
-- Use case: Measure reliability — key SLA metric
-- ----------------------------
SELECT
    p.provider_name,
    r.origin_city || ' → ' || r.dest_city AS lane,
...
-- Use case: How much do rates vary between providers on the same lane?
-- Identifies where you have negotiation leverage
-- ----------------------------
SELECT
    r.origin_city || ' → ' || r.dest_city AS lane,
    q.weight_band_kg,
    ...
    STRING_AGG(p.provider_name || ': €' || q.base_rate_eur_per_kg::TEXT, ' | ' ORDER BY q.base_rate_eur_per_kg) AS provider_rates
FROM quotes q
JOIN routes    r ON q.route_id    = r.route_id
JOIN providers p ON q.provider_id = p.provider_id
...
-- Use case: At what volume does each provider become cheapest?
-- Simulates 500kg and 2000kg shipment scenarios
-- ----------------------------
WITH scenarios AS (
...
-- Use case: Identify which delay reasons drive the most disruption
-- ----------------------------
SELECT
...
-- ----------------------------
-- QUERY 6: Monthly trend — average cost per kg and on-time % per provider
-- Use case: Is performance improving or degrading over time? (Power BI trend line source)
-- ----------------------------
SELECT
    TO_CHAR(ph.month_year, 'YYYY-MM') AS month,
    p.provider_name,
    r.origin_city || ' → ' || r.dest_city AS lane,
    ph.total_shipments,
    ph.on_time_count,
...
-- Use case: Final recommendation engine for procurement decision
-- ----------------------------
...
normalised AS (
    SELECT
        ...
        avg_on_time_pct,
        avg_lead_days,
        -- Normalise cost: lower is better → invert scale to 0-100
        ...
        -- Normalise reliability: higher is better
        ...
        -- Normalise lead time: lower is better → invert
...
-- Flags providers whose rolling 3-month on-time % drops below SLA threshold
-- Designed to simulate what a supply chain analyst would automate
-- ============================================================

CREATE OR REPLACE FUNCTION check_sla_breach_alerts()
RETURNS TABLE (
    provider_name     VARCHAR,
    lane              TEXT,
    rolling_3m_otd    NUMERIC,
...
BEGIN
    ...
    WITH rolling_otd AS (
        SELECT
            p.provider_name,
            r.origin_city || ' → ' || r.dest_city AS lane_label,
            p.sla_threshold,
            ROUND(
...
-- Execute the SLA breach alert function
SELECT * FROM check_sla_breach_alerts();


-- ============================================================
-- SECTION 5: VIEWS FOR POWER BI CONNECTIVITY
-- These views are designed to be imported directly into Power BI
-- as data sources — no transformation needed in Power Query
-- ============================================================

-- View 1: Provider Scorecard (Power BI KPI cards + radar chart source)
...
SELECT
    ...
    SUM(ph.sla_breached::INT)             AS sla_breach_months
FROM performance_history ph
JOIN providers p ON ph.provider_id = p.provider_id
JOIN routes    r ON ph.route_id    = r.route_id
GROUP BY p.provider_name, lane;

-- View 2: Monthly Trend (Power BI line chart source)
...
-- View 3: Shipment Detail (Power BI table + drill-through source)
...
SELECT
    ...
    s.promised_delivery,
    s.actual_delivery,
    (s.actual_delivery - s.promised_delivery) AS delay_days,
    s.total_cost_eur,
...
-- ============================================================
-- END OF FILE
-- To use: Run in PostgreSQL (pgAdmin / DBeaver / psql)
